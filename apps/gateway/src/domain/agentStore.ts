  ...
  return {
    ...
    "IDENTITY.md": agent.id === "coding-agent"
      ? [
          ...
          "",
          "## Self-review via spawned subagent (mandatory after non-trivial edits)",
          "After verification passes (typecheck / tests green) and BEFORE you tell the user the work is done, spawn a code-review subagent and incorporate its findings:",
          "",
          "1. Call `sessions_spawn` with these fields:",
          "   - `agentId`: same as your own (a fresh session of yourself, framed as reviewer).",
          "   - `label`: `\"review\"`.",
          "   - `title`: short — e.g. `\"Review: <one-line scope>\"`.",
          "   - `message`: a single block containing:",
          "     - `Read the skill at \"code-reviewer-role\" and follow its rules — you are a reviewer, not an implementer.`",
          "     - The user's original request, verbatim or paraphrased.",
          "     - The list of files you touched (one per line, absolute paths).",
          "     - The verification command(s) you ran and the success line(s) from the output.",
          "     - One-sentence intent: \"Goal of this change was: …\".",
          "2. Wait for the subagent's reply via `sessions_yield` (with the same `parentRunId` the spawn returned).",
          "3. Parse the VERDICT line. On `BLOCK`: address every CRITICAL and HIGH finding, re-verify, then spawn another review pass. On `APPROVED-WITH-WARNINGS`: address MEDIUM findings if they're real correctness issues (skip pure style nits) and note in your final reply which warnings you took and which you skipped + why. On `APPROVED`: report success to the user.",
          "4. Only declare the work done after a clean review (or an APPROVED-WITH-WARNINGS where you've explicitly justified the skips).",
          "",
