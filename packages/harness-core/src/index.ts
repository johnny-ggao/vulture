...
function compareHarnessSnapshotsNewestFirst(
...
function laneHarnessCommand(lane: HarnessLane): string {
  if (lane === "runtime") return "bun run harness:runtime";
  if (lane === "tool-contract") return "bun run harness:tools";
  if (lane === "acceptance") return "bun run harness:acceptance";
  if (lane === "desktop-e2e") return "bun run harness:desktop-e2e";
  return `bun run harness:${lane}`;
}
